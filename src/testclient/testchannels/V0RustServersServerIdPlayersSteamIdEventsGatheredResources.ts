...
export function publish(
  ...
  return new Promise < void > (async (resolve, reject) => {
    try {
      ...
      nc.publish(`v0.rust.servers.${server_id}.players.${steam_id}.events.gatheredResources`, dataToSend, options);
      resolve();
    } catch (e: any) {
...
export function jetStreamPublish(
  ...
  return new Promise < void > (async (resolve, reject) => {
    try {
      let dataToSend: any = message.marshal();
      dataToSend = codec.encode(dataToSend);
      js.publish(`v0.rust.servers.${server_id}.players.${steam_id}.events.gatheredResources`, dataToSend, options);
      resolve();
    } catch (e: any) {
      reject(NatsTypescriptTemplateError.errorForCode(ErrorCode.INTERNAL_NATS_TS_ERROR, e));
    }
