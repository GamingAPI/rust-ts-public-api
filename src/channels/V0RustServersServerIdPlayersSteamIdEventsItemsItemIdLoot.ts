...
/**
 * Internal functionality to setup jetstrema pull on the `v0/rust/servers/{server_id}/players/{steam_id}/events/items/{item_id}/loot` channel 
 * 
 * @param onDataCallback to call when messages are received
...
export function jetStreamPull(
  ...
  const stream = `v0.rust.servers.${server_id}.players.${steam_id}.events.items.${item_id}.loot`;
  (async () => {
    ...
    const itemIdParam = "" + channel.substring(0, itemIdEnd);
    let receivedData: any = codec.decode(msg.data);
    onDataCallback(undefined, ServerPlayerItemLoot.unmarshal(receivedData), serverIdParam, steamIdParam, itemIdParam, msg);
  })();
}
/**
 * Internal functionality to setup jetstream push subscription on the `v0/rust/servers/{server_id}/players/{steam_id}/events/items/{item_id}/loot` channel 
 * 
 * @param onDataCallback to call when messages are received
 * @param nc to subscribe with
 * @param codec used to convert messages
...
export function jetStreamPushSubscribe(
  ...
  return new Promise(async (resolve, reject) => {
    try {
      let subscription = js.subscribe(`v0.rust.servers.${server_id}.players.${steam_id}.events.items.${item_id}.loot`, options);
      (async () => {
...
/**
 * Internal functionality to setup jetstream pull subscription on the `v0/rust/servers/{server_id}/players/{steam_id}/events/items/{item_id}/loot` channel 
 * 
 * @param onDataCallback to call when messages are received
 * @param nc to subscribe with
 * @param codec used to convert messages
...
export function jetStreamPullSubscribe(
  ...
  return new Promise(async (resolve, reject) => {
    try {
      const subscription = await js.pullSubscribe(`v0.rust.servers.${server_id}.players.${steam_id}.events.items.${item_id}.loot`, options);
      (async () => {
        for await (const msg of subscription) {
          const unmodifiedChannel = `v0.rust.servers.{server_id}.players.{steam_id}.events.items.{item_id}.loot`;
          let channel = msg.subject;
...
/**
 * Internal functionality to setup jetstrema fetch on the `v0/rust/servers/{server_id}/players/{steam_id}/events/items/{item_id}/loot` channel 
 * 
 * @param onDataCallback to call when messages are received
